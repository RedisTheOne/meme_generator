...
def drawTemplateFour(firstText, secondText, thirdText):
    #CHECK IF TEMPLATE FOLDER IS CREATED
    checkIfTempFolderExists()

    #LOAD THE IMAGE, AND GET THE DIMENSIONS
    img = Image.open('assets/template4.jpg')
    imgWidth, imgHeight = img.size
    draw = ImageDraw.Draw(img)
    font = ImageFont.truetype("font_family.ttf", 35)
    ...
    while isValid == False:
        ...
        firstTextWidth, firstTextHeight = draw.textsize(firstText, font=font)
        isValid = firstTextWidth < (imgWidth / 2 - 40)
    draw.text((20, ((imgHeight / 6)) - (firstTextHeight / 2)), firstText, fill="black", font=font)
    ...
    while isValid == False:
        ...
        secondTextWidth, secondTextHeight = draw.textsize(secondText, font=font)
        isValid = secondTextWidth < (imgWidth / 2 - 40)
    draw.text((20, ((imgHeight / 2)) - (secondTextHeight / 2)), secondText, fill="black", font=font)
    ...
    while isValid == False:
        ...
        thirdTextWidth, thirdTextHeight = draw.textsize(thirdText, font=font)
        isValid = thirdTextWidth < (imgWidth / 2 - 40)
    draw.text((20, ((imgHeight / 1.2)) - (thirdTextHeight / 2)), thirdText, fill="black", font=font)
